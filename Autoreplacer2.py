...
# VARIABLES
# ---------------------------------------------------------
COMBINATION_S = {keyboard.Key.ctrl_l, keyboard.KeyCode(char='`')}
...
def nofile():
    ...
    worksheet.write(
        'C4', '(qwe and QWE and Qwe and йце (cyrillic keyboard layout) - the same one keyword and has one 1st replacement)', cf2)
    worksheet.write(
        'C5', 'Keywords must not include gaps(space)', cf2)
    workbook.close()
    root = Tk()
    root.configure(bg='#1C2833')
    w = Label(root, text='\n1) A1 (Keyword) and B1 (Replacement) must not be changed \n\n2) If cell Ax is not empty, Bx must also not be empty, and vice versa\n\n3) Keywords are not sensitive to capital and small letters, and keyboard layout (qwe=QWE=йце)\n4) Keywords must not include gaps(space)', font=(
        "Arial", 15), fg='#7FB3D5', bg='#1C2833')
    w.pack(side=TOP, anchor=W)
    w2 = Label(root, text='!!!Please check the keyboard layout before use (If cyrillic - replacement will be printed by cyrillic letter, and vice versa', font=(
        "Arial", 15), fg='red', bg='#1C2833')
...
def menu():
    ...
    lbl3 = Label(
        window, text=" 1st: press crl+` key on keyboard to activate autoreplacement \n 2nd: when * is printed, press combination of keys which creates keyword \n 3rd: press Space to apply replacement ", font=("Arial", 10), bg='#1C2833', fg='#AAAAAA')
    lbl3.pack(side=TOP)
...
def on_press(key):
    '''on_press function for replacement. Activated by `symbol, disable by shift+space button. If combination of letters meets combination in excel it will be removed and changed'''
    global opt
    global typed_keys
    global listening
    global replacements
